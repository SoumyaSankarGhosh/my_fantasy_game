...
@Component({
  selector: 'app-image-category-modal',
...
export class ModalComponent implements OnInit,OnChanges {
  ...
  ngOnChanges(changes:SimpleChanges) {
    if(Object.keys(changes.modalPassData.currentValue).length){
      this.image_category = changes.modalPassData.currentValue;
      this.isSubmited = false;
      this.icon_error = false;
      this.icon = null;
      if(this.image_category.action == "Edit"){
        this.createForm = this.FB.group({
          id:[this.image_category.id],
          name:[this.image_category.name,[Validators.required]],
          icon:[]
        });
        this.previewImageUrl = environment.image_category_image_url+this.image_category.icon;
      } else {
        this.createForm = this.FB.group({
          id:[''],
  ...
  submitForm(){
    ...
    if(this.createForm.valid){
      ...
      if(this.createForm.value.id){
        ...
        this.apiService.imageCategoryUpdate(formData).subscribe(success=>{
          if(success.status){
         ...
         this.apiService.imageCategoryCreate(formData).subscribe(success=>{
          if(success.status){
            this.toastr.success('Success!', success.msg);
            this.modalDialog.hide();
            this.dataTableRefresh.emit({});
