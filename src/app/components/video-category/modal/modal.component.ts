...
@Component({
  selector: 'app-video-category-modal',
...
export class ModalComponent implements OnInit,OnChanges {
  ...
  ngOnChanges(changes:SimpleChanges) {
    if(Object.keys(changes.modalPassData.currentValue).length){
      this.video_category = changes.modalPassData.currentValue;
      this.isSubmited = false;
      this.icon_error = false;
      this.icon = null;
      if(this.video_category.action == "Edit"){
        this.createForm = this.FB.group({
          id:[this.video_category.id],
          name:[this.video_category.name,[Validators.required]],
          icon:[]
        });
        this.previewImageUrl = environment.video_category_image_url+this.video_category.icon;
      } else {
        this.createForm = this.FB.group({
          id:[''],
  ...
  submitForm(){
    ...
    if(this.createForm.valid){
      ...
      if(this.createForm.value.id){
        ...
        this.apiService.videoCategoryUpdate(formData).subscribe(success=>{
          if(success.status){
         ...
         this.apiService.videoCategoryCreate(formData).subscribe(success=>{
          if(success.status){
            this.toastr.success('Success!', success.msg);
            this.modalDialog.hide();
            this.dataTableRefresh.emit({});
